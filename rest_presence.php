...
{
	trigger_error("/presence Method=".$_RPC_REQUEST->Method);
}
...
if(!preg_match("/^[A-Za-z_]*$/", $_RPC_REQUEST->Method))
{
	http_response_code("400");
	header("Content-Type: text/plain");
	echo "Misformatted method";
	exit;
}
else if(!file_exists("presence/rpc_".$_RPC_REQUEST->Method.".php"))
{
	http_response_code("400");
	header("Content-Type: text/plain");
...
{
	require_once("presence/rpc_".$_RPC_REQUEST->Method.".php");
}
